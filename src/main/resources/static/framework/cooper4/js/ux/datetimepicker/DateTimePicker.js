...
Ext.define('MyApp.ux.datetimepicker.DateTimePicker', {
    ...
    renderTpl: [
        '<div id="{id}-innerEl" data-ref="innerEl">',
            ...
            '</table>',
            '<tpl if="showToday">',
                '<div id="{id}-footerEl" role="presentation" style="background-color:#FFFFFF;border-top:0px solid #99BCE8;">{%this.renderHour(values, out)%}{%this.renderMinute(values, out)%}{%this.renderSecond(values, out)%}<center>{%this.renderTodayBtn(values, out)%}</center></div>',
            '</tpl>',
        '</div>',
        {
